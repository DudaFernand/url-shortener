...
- Rate limiting via Redis para proteger endpoints de criação e redirect
- Listagem paginada de links com filtro por URL
...
Cliente
  │
  ▼
RateLimitFilter (limite por IP/usuário)
  │
  ▼
JwtAuthFilter (valida JWT)
  │
  ├──► AuthController        (/auth/*)
  ├──► LinkController        (/links — autenticado)
  └──► RedirectController    (/{code} — público)
         │
         ▼
...
ExpiredLinkCleanupJob (@Scheduled)
  ├──► evict cache Redis
  └──► delete links expirados no Postgres
```

### Estrutura de pacotes

```
com.mariafernandes.urlshortener/
├── config/       → CacheConfig
├── controller/   → AuthController, LinkController, RedirectController
├── domain/       → ShortUrl, User
├── dto/          → requests e responses
├── exception/    → GlobalExceptionHandler, LinkExpiredException
├── job/          → ExpiredLinkCleanupJob
├── repository/   → ShortUrlRepository, UserRepository
├── security/     → JWT, RateLimitFilter, SecurityConfig
└── service/
    ├── CodeGenerator.java
    ├── ShortUrlService.java, AuthService.java
...
Links podem ter TTL definido na criação (`expiresInDays`) ou herdarem o padrão global (`app.link.default-expiration-days`). Na leitura, links expirados retornam `410 Gone` antes mesmo do job rodar. O job agendado em `job/ExpiredLinkCleanupJob` remove registros expirados do Postgres e evicta as entradas correspondentes no Redis.
...
1. Métricas de observabilidade (Prometheus/Grafana)
